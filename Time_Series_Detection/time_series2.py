...
#Channelquery = '少康戰情室'
Channelquery = '關鍵時刻'
Channelquery = input("關鍵時刻、少康戰情室、比特王出任務，Choose one to search:")
...
def Build_Top_Level_User_Id_List(comment_count, user_count):
...
def String_Compare():
	...
	print('string_compare_ratio='+str(len(string_compare_ratio)))
	print('final_user='+str(len(final_user)))
	final_df = pd.DataFrame()
	final_df = final_df.append(string_compare_ratio,ignore_index=True)
	final_df = final_df.transpose()
	print(final_df)

	final = pd.DataFrame()
	final = final.append(final_comment,ignore_index=True)
